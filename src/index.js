...
class WorldMap extends Component {
  constructor() {
    ...
    const typhoonPaths = [];
    const startPoints = [];
    parsedWeatherData.forEach((pathData) => {
  ...
  moveDash = (canvasCtx, canvasTyphoonMarkerCtx, typhoonId, typhoonPath, length, marker, frac, dir = -1) => {
    ...
    canvasTyphoonMarkerCtx.arc(p.x, p.y, this.radius[typhoonId], 0, Math.PI * 2, true);
    canvasTyphoonMarkerCtx.closePath();
    canvasTyphoonMarkerCtx.stroke();
    // Inner Typhoon marker
    canvasTyphoonMarkerCtx.beginPath();
    canvasTyphoonMarkerCtx.setLineDash([0]);
    canvasTyphoonMarkerCtx.lineDashOffset = 0;
    canvasTyphoonMarkerCtx.lineWidth = 1;
    canvasTyphoonMarkerCtx.fillStyle = 'rgba(53, 247, 14,0.8)';
    canvasTyphoonMarkerCtx.strokeStyle = 'rgba(53, 247, 14,0.8)';
    canvasTyphoonMarkerCtx.arc(p.x, p.y, 5, 0, Math.PI * 2, true);
    canvasTyphoonMarkerCtx.closePath();
    canvasTyphoonMarkerCtx.stroke();
    canvasTyphoonMarkerCtx.fill();
    // add marker
    typhoonPath.forEach((path, pathIdx) => {
      const pixelLocSource = this.projection()(path.coordinates);
      /* const nextIdx = (pathIdx === typhoonPath.length - 1) ? 0 : pathIdx + 1; */
      /* const nextPixelLocSource = this.projection()(typhoonPath[nextIdx].coordinates); */
      if (!this.renderedPath[Math.floor(p.x)] && Math.floor(p.x) === Math.floor(pixelLocSource[0]) && Math.floor(p.y) === Math.floor(pixelLocSource[1])) {
        this.renderedPath[Math.floor(p.x)] = true;
        const over24hr = this.timeRangeCalculate(path.time, 24);
        this.radius[typhoonId] = this.distanceCalculate(pixelLocSource, path.coordinates, path.radius);/* 
                this.nextRadius = this.distanceCalculate(nextPixelLocSource, typhoonPath[nextIdx].coordinates, typhoonPath[nextIdx].radius); */
        const radius = this.radius[typhoonId];
        canvasCtx.globalCompositeOperation = 'source-over';
        canvasCtx.beginPath();
        canvasCtx.lineWidth = 1;
        canvasCtx.fillStyle = 'rgba(53, 247, 14,0.8)';
        canvasCtx.arc(p.x, p.y, 5, 0, Math.PI * 2, true);
        canvasCtx.fill();
        canvasCtx.closePath();
        ...
        if (path.status === 'curr') {
          ...
          canvasCtx.arc(p.x, p.y, radius, 0, Math.PI * 2, true);
          canvasCtx.stroke();
          canvasCtx.fill();
          canvasCtx.closePath();
        }
        if (path.status === 'fcst') {
          canvasCtx.beginPath();
          canvasCtx.lineWidth = 1;
          canvasCtx.fillStyle = over24hr ? 'none' : 'rgba(53, 247, 14,0.7)';
          canvasCtx.strokeStyle = over24hr ? 'rgba(53, 247, 14,0.8)' : '#feff00';
          canvasCtx.setLineDash([5, 5]);
          canvasCtx.arc(p.x, p.y, over24hr ? 5 : radius, 0, Math.PI * 2, true);
          canvasCtx.stroke();
          canvasCtx.fill();
          canvasCtx.closePath();
        }
  ...
  defineLine(canvasCtx, canvasTyphoonMarkerCtx, typhoonPath, marker, typhoonId) {
  ...
  updateLine(canvasCtx, canvasTyphoonMarkerCtx, typhoonPath, length, marker, typhoonId) {
    // define the line
    this.defineLine(canvasCtx, canvasTyphoonMarkerCtx, typhoonPath, marker, typhoonId);
  ...
  renderLine(canvasCtx, canvasTyphoonMarkerCtx, typhoonPath, marker, typhoonId) {
    ...
    this.lineInterval[typhoonId] = requestAnimationFrame(this.updateLine.bind(this, canvasCtx, canvasTyphoonMarkerCtx, typhoonPath, length, marker, typhoonId));
  }
